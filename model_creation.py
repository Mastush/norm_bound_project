from tensorflow.keras.layers import Input, Conv2D, MaxPool2D, Flatten, Dense, MaxPooling2D, Activation, Dropout, BatchNormalization
...
def get_convolutional_model(input_shape, num_of_classes, num_of_conv_layers, num_of_channels, kernel_size, stride,
    ...
    :param last_activation: (activation function) the activation function to be activated on the outputs of the
                             last (output) layer
    :param optimizer: (tensorflow.keras optimizer) the wanted optimizer for training
    :param loss_function: (tensorflow.keras loss function) the wanted loss function fot training
...
def alexnet(metrics=[], dropout=False):
    input_layer = Input(constants.IMAGENET_INPUT_SHAPE)
